...
/**
 * @notice Implement _isApprovedOrOwner
 */
 ...
 * */
    ...
    function _setTokenURI(uint256 _tokenId, string memory _tokenURI) internal {
        require(_exists(tokenId), "ERR: cannot set URI of non-existent token");
 ...
 */
    ...
    {
        require(owner != address(0), "ERR: address zero is not a valid owner.");
    ...
    returns (address owner)
    {
        ...
        require(owner != address(0), "ERR: invalid token ID.");
        return owner;
    }
    ...
    {
        require(_isApprovedOrOwner(msg.sender, tokenId), "ERR: caller is not the owner or approved.");
    ...
    {
        address owner = _ownerOf(tokenId);
        require(to != owner, "ERR: cannot approve to current owner.");

        require(
            msg.sender == owner || isApprovedForAll(owner, msg.sender),
            "ERR: approve caller is not token owner or approved operator."
        );
...
function _checkOnERC721Received(
        ...
        if (to.isContract()) {
            try IERC721Receiver(to).onERC721Received(msg.sender, from, tokenId, data) returns (bytes4 retval) {
                ...
                if (reason.length == 0) {
                    revert("ERR: transfer to non ERC721Receiver.");
                } else {
...
*/
    function _safeTransfer(
        ...
        require(_checkOnERC721Received(_from, _to, _tokenId, _data), "ERR: transfer to non ERC721Receiver.");
    }    
...
function _transfer(address _from, address _to, uint256 _tokenId) internal {
    ...
    require(_to != address(0), "ERR: Only transfer to valid address.");
...
function _safeMint(
        ...
        require(
            ...
            "ERR: transfer to non ERC721Receiver."
        );
    }
...
function _mint(address _to, uint256 _tokenId) internal {
    require(_to !== address(0), "ERR: Address does not exist.");
    require(!exists(_tokenId), "ERR: Token already exists.");
...
internal 
{
    require(_owner != _operator, "ERR: Owner cannot be the operator.");
...
function _hasBeenMinted(uint256 _tokenId) internal view {
    require(_exists(tokenId), "ERR: Token is not minted.")
}
